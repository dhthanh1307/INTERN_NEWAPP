...
export const Carousel: React.FC<CarouselProps> = ({ slides }) => {
    ...
    return (
        <div className="relative mx-auto w-full max-w-3xl bg-white">
            ...
            <div className="absolute bottom-0 left-1/2 flex -translate-x-1/2 items-center space-x-0.5">
                {slides.map((_, index) => (
                    <button
                        key={index}
                        onClick={() => scrollTo(index)}
                        className={`h-0.5 w-3 rounded-full transition-colors ${index === selectedIndex ? "bg-normal" : "bg-gray-200"}`} />
                ))}
                <button
                    className="h-0.5 w-0.5 rounded-full bg-gray-200  transition-colors" />
            </div>
        </div>
